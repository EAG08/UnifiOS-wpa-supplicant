...
This guide was written specifically for the UXG Max.
...
> NOTE: Take note of your Unifi router's WAN port interface name. In the rest of the guide, I'll be using `eth4` because that is the WAN interface for the UXG Max. If using another device, replace the interface name appropriately.
...
## Spoof MAC address
We'll need to spoof the MAC address on the WAN port (interface `eth4` on the UXG-Max) to successfully authenticate with ATT with our certificates.
...
- `-i eth4` Specifies `eth4` (UXG-Max WAN port) as the interface
...
...
Dec 29 23:20:00 UXG-Max wpa_supplicant[6845]: eth4: CTRL-EVENT-EAP-SUCCESS EAP authentication completed successfully
```
</details>
